...
def get_resource(group, name):
    return os.path.join(os.getcwd()+"/scripts/", group, name)
...
def get_bokeh(df, linear = False, show_muts = False):
    ...
    df = calc_level(df)
    df['level'] = np.where(df['Type'].isin(['insert','deletion',"mismatch"]),0, df['level'])
    ...
    #st.write(df)
    TOOLTIPS='<font size="3"><b>@label</b> — @Type </font>' 
...
def save_plot(inDf, name):
    bokeh_plot = get_bokeh(inDf)
    bokeh_plot.sizing_mode = "fixed"
    html = file_html(bokeh_plot, resources = CDN, title = name)
